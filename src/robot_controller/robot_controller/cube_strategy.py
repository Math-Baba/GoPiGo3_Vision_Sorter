...
class CubeStrategy(Node):
    ...
    def loop(self):
        ...
        if self.state == 'CHERCHER':
            ...
            self.state_pousser()
        elif self.state == 'POUSSER_DROIT':
            self.state_pousser_droit()
    ...
    def state_pousser(self):
        """Phase 1: Tourner vers la zone en poussant doucement (dribble)"""
        ...
        # Deja oriente ou timeout 4s -> pousser tout droit
        if abs(angle_error) < math.radians(20) or elapsed > 4.0:
            self.push_start = time.time()  # reset timer pour la poussee droite
            self.get_logger().info(
                f'[POUSSER] {color} oriente! heading={math.degrees(self.robot_heading):.0f}° -> POUSSER_DROIT'
            )
            self.change_state('POUSSER_DROIT')
            return

        # Dribble: avancer doucement + tourner
        turn_speed = 0.25 if abs(angle_error) > math.radians(30) else 0.15
        if angle_error > 0:
            self.cmd(linear=0.15, angular=turn_speed)
        else:
            self.cmd(linear=0.15, angular=-turn_speed)

        self.get_logger().info(
            f'[POUSSER] Dribble {color} error={math.degrees(angle_error):.0f}°'
        )

    def state_pousser_droit(self):
        """Phase 2: Pousser tout droit vers la zone"""
        elapsed = time.time() - self.push_start
        color = self.push_color or 'unknown'
        ...
        if remaining <= 0:
            ...
            self.total_sorted += 1
            self.get_logger().info(
                f'[POUSSER_DROIT] {color} livre! Total: {self.total_sorted} '
